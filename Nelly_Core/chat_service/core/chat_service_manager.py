...
import chat_service.utils.server as server_utils
from chat_service.core.world_runner import ChatServiceWorldRunner
...
class ChatServiceManager(ABC):
    ...
    def _parse_config(self, opt: Opt):
        ...
        self.world_runner = ChatServiceWorldRunner(
            self.runner_opt, self.world_module, self.max_workers, self, self.debug
    ...
    def _on_first_message(self, message: Dict[str, Any]):
        ...
        if self.opt.get('password', None) is not None:
    ...
    def observe_message(
        ...
        """
        print("calling observe message" + text +" From:" + receiver_id)
