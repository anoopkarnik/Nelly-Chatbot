...
from core.lr_scheduler import ParlAILRScheduler
...
class TorchAgent(ABC, Agent):
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        logging.info("Model is init in parallel mode : " + str(self.model_parallel))
        ...
        if shared is None:
            ...
            if opt.get('fp16') or opt.get('force_fp16_tokens'):
                ...
                from utils.torch import FP16_PAD_SIZE
    ...
    def _get_embtype(self, emb_type):
        ...
        if emb_type.startswith('glove'):
            init = 'glove'
            from zoo.glove_vectors.build import download

            embs = download(self.opt.get('datapath'))
        elif emb_type.startswith('fasttext_cc'):
            init = 'fasttext_cc'
            from zoo.fasttext_cc_vectors.build import download

            embs = download(self.opt.get('datapath'))
        elif emb_type.startswith('fasttext'):
            init = 'fasttext'
            from zoo.fasttext_vectors.build import download
    ...
    def load(self, path: str) -> Dict[str, Any]:
        ...
        states = torch.load(
            path, map_location=lambda cpu, _: cpu, pickle_module=utils.pickle
        )
        if 'model' in states:
    ...
    def act(self):
        ...
        response = self.batch_act(self.observation)
        if response is not None and len(response) > 1:
            response = self.self_observe(response[0])
        return response
