...
from core.metrics import SumMetric, AverageMetric, BleuMetric, FairseqBleuMetric
...
class TorchGeneratorAgent(TorchAgent, ABC):
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        if shared is None and is_distributed():
            ...
            logging.info("Torch Generator agent - Device Ids " + len(device_ids))
            self.model = torch.nn.parallel.DistributedDataParallel(
                self.model, device_ids=device_ids, broadcast_buffers=False
            )
            logging.info("successfully loaded the model via torch.nn.parallel.distributedDataParallel")
        self.reset()
